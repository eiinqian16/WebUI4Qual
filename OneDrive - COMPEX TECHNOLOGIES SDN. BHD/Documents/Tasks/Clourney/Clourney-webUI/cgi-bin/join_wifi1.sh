...
EXISTING_IFACE=$(uci show wireless | grep "wireless.default_radio[0-9].bssid='$BSSID'" | awk -F'=' '{print $1}')

if [ -n "$EXISTING_IFACE" ]; then
    echo "[$(date)] Found existing iface for BSSID $BSSID: $EXISTING_IFACE" >> "$LOG_FILE"
    EXISTING_IFACE=$(echo "$EXISTING_IFACE" | sed 's/\.[^.]*$//')
else
    echo "[$(date)] No existing iface found. Creating a new one..." >> "$LOG_FILE"

    MAX_IFACE_INDEX=$(uci show wireless | grep -o "wireless.default_radio[0-9]" | grep -o "[0-9]\+" | sort -nr | head -n 1)
    ...
    EXISTING_IFACE="wireless.default_radio[$NEW_IFACE_INDEX]"
fi
...
if [ "$ENCRYPTION" = "Open" ]; then
    ...
    uci set $EXISTING_IFACE.encryption="psk2+tkip+ccmp"
    ...
    uci set $EXISTING_IFACE.key="$PASSWORD"
    uci set $EXISTING_IFACE.ieee80211w=2
