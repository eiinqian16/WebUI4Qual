...
uci -q delete wireless.qcawifi
echo "Removed non-MLO 11be operation mode." >> $LOG_FILE
...
for WIFI_DEV in $WIFI_LIST; do
    ...
    uci -q delete wireless.@wifi-iface[$IFACE_INDEX].sae_password
...
for dev in wifi0 wifi1 wifi2 wifi3; do
    if ! echo "$WIFI_LIST" | grep -qw "$dev"; then
        echo "Disabling unused device $dev" >> $LOG_FILE
