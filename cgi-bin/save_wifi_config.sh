...
get_current_band() {
    local dev=$1
    local device=$(get_iface_for_radio $dev)
...
delete_other_ifaces() {
    ...
    for iface in $(uci show wireless | grep "wireless.default_radio" | grep "$device" | awk -F'.' '{print $2}'); do
...
SUPPORTED_BANDS_FILE="$CURRENT_BAND"
...
if [ "$CREATE_NEW" = "true" ]; then
    MAX_IFACE_INDEX=$(uci show wireless | sed -n 's/.*wireless\.default_radio\([0-9]\+\).*/\1/p' | sort -nr | head -n 1)
    ...
    NEW_IFACE=$(uci add wireless default_radio)
    ...
    IFACE_NAME="default_radio[$NEW_IFACE_INDEX]"
else
...
if [ "$ENCRYPTION_TYPE" = "sae" ]; then
    ...
    uci set wireless.$IFACE_NAME.ieee80211w=2
...
if [ "$HWMODE" == "11b" ]; then 
    uci set wireless.$DEVICE.legacy_rates=1
    uci set wireless.$DEVICE.basic_rate='1000 2000 5500 11000'
    uci set wireless.$DEVICE.supported_rates='1000 2000 5500 11000'
else
    uci del wireless.$DEVICE.legacy_rates
    uci del wireless.$DEVICE.basic_rate
    uci del wireless.$DEVICE.supported_rates
fi
