...
LOG_FILE="/tmp/rg255a_config.log"
...
get_cur_slot() {
    local resp=$(at_cmd "AT+QDSIM?")
    # RG255A: QDSIM: 0 means Slot 1, 1 means Slot 2
    local val=$(echo "$resp" | grep "+QDSIM:" | awk '{print $2}' | tr -dc '0-9')
    [ -n "$val" ] && echo $((val + 1))
}
...
set_sim_slot() {
    ...
    # RG255A uses 0-based indexing for QDSIM (0=Slot 1, 1=Slot 2)
    local cmd_val=$((target_slot - 1))

    log "Setting SIM slot to $target_slot (AT+QDSIM=$cmd_val)"
    local resp=$(at_cmd "AT+QDSIM=$cmd_val" 5)
...
#main
log "Starting RG255A APN configuration"
