...
model=$(cat /tmp/modem_model 2>/dev/null || echo "RG255A")
...
# Derive Operator and RAT from QENG directly
operator="${mcc}${mnc}"
[ -z "$mcc" ] && operator="No operator detected"

qeng_rat=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $3}' | tr -d '" ')
case "$qeng_rat" in
    "LTE")     rat="7" ;;
    "NR5G-SA") rat="11" ;;
    *)         rat="No band found" ;;
...
if [ "$rat" == "7" ]; then 
    band=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $10}' | tr -dc '0-9-')
    rsrp=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $14}' | tr -dc '0-9-')
    rsrq=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $15}' | tr -dc '0-9-')
    snr=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $17}' | tr -dc '0-9.-')

elif [ "$rat" == "11" ]; then
    band=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $11}' | tr -dc '0-9-')
    rsrp=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $13}' | tr -dc '0-9-')
    rsrq=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $14}' | tr -dc '0-9-')
    snr=$(echo "$serving" | grep '^+QENG: "servingcell"' | awk -F ',' '{print $15}' | tr -dc '0-9.-')
fi
