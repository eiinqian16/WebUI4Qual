...
extract_sys_info() {
    ...
    firmwarePath="/www/webUI/ov/fw_version"
    modelPath="/www/webUI/ov/model"

    json_output="{"
    model=$(cat "$modelPath" 2>/dev/null || echo "Unknown")
    fw_version=$(cat "$firmwarePath")
    ...
    [ -n "$fw_version" ] && json_output="${json_output},\"fw_version\":\"${fw_version}\""
